    ...
    <Layout title="admin | m3mento">
      <div className="flex justify-center mt-60">
        <div className="bg-red-100 rounded-lg flex flex-col p-12 text-center">
          ...
          <button onClick={openSale} className={ButtonStyle} disabled={isSaleActive}>
            Open Sale
          </button>
          <button onClick={closeSale} className={ButtonStyle} disabled={!isSaleActive}>
            Close Sale
          </button>
          <h2>Scan User</h2>
          <input onChange={handleScan} />
          {isOwner && (
            <button onClick={checkIn} className={ButtonStyle}>
              Check In
            </button>
          )}
        </div>
